...
module.exports = class Rule {
  ...
  _stringCheck(path, value) {
    if (!_.isUndefined(this._min)) {
      if (this._min > value.length) {
        throw { id: 6, path, expected: this._min, actual: value.length };
      }
    }

    if (!_.isUndefined(this._max)) {
      if (this._max < value.length) {
        throw { id: 7, path, expected: this._max, actual: value.length };
      }
    }

    if (!_.isUndefined(this._len)) {
      if (this._len !== value.length) {
        throw { id: 5, path, expected: this._len, actual: value.length };
      }
    }

    if (!_.isUndefined(this._oneOf)) {
      if (!~this._oneOf.indexOf(value)) {
        throw { id: 8, path, expected: this._oneOf, actual: value };
      }
    }

    if (!_.isUndefined(this._match)) {
      if (!this._match.test(value)) {
        throw { id: 8, path, expected: this._match, actual: value };
      }
    }
  }
  ...
  _numberCheck(path, value) {
    if (!_.isUndefined(this._min)) {
      if (this._min > value) {
        throw { id: 9, path, expected: this._min, actual: value };
      }
    }

    if (!_.isUndefined(this._max)) {
      if (this._max < value) {
        throw { id: 10, path, expected: this._max, actual: value };
      }
    }
  ...
  _arrayCheck(path, value) {
    ...
    if (!_.isUndefined(this._schema)) {
      ...
      if (_.isPlainObject(this._schema)) {
        for (let item of value) {
  ...
  check(value, path) {
    ...
    if (this._type !== type.identify(value)) {
      if (this._null) {
        throw { id: 4, path, expected: this._type, actual: type.identify(value) };
      } else {
