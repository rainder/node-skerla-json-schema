...
module.exports = class Rule {
  ...
  _stringCheck(path, value) {
    ...
    if (!_.isUndefined(this._specs.oneOf)) {
      const oneOf = [...new Set(this._specs.oneOf)];
      if (!~oneOf.indexOf(value)) {
        throw error.ERROR_VALUE_ONE_OF(path, oneOf, value);
      }
    }
   ...
   * @private
   */
  _functionCheck(path, value) {
    if (!_.isUndefined(this._specs.typeOf)) {
      const actualType = type.identify(value);
      const expectedTypes = this._specs.typeOf.map((item) => type.constructorToString(item));

      if (!expectedTypes.includes(actualType)) {
        throw error.ERROR_VALUE_TYPE_ONE_OF(path, expectedTypes, actualType);
      }
    }

    if (!_.isUndefined(this._specs.fn)) {
      this._specs.fn(value, path);
    }
  }
  ...
  _arrayCheck(path, value) {
    ...
    if (!_.isUndefined(this._specs.oneOf)) {
      for (let item of value) {
        const oneOf = [...new Set(this._specs.oneOf)];
        if (!~oneOf.indexOf(item)) {
          throw error.ERROR_VALUE_ONE_OF(path, oneOf, item);
        }
      }
    }
  ...
  required(isRequired = true) {
    this._specs.required = isRequired;
    return this;
  }
  ...
  check(value, path) {
    ...
    if (this._specs.required && value === undefined) {
      throw error.ERROR_REQUIRED(path);
    }

    if (this._specs.type === type.TYPE_FUNCTION) {
      this._functionCheck(path, value);
    } else if (this._specs.type !== type.identify(value)) {
      if (this._specs.null) {
